...
namespace jsb
{
    ...
    String JavaScriptRuntime::handle_source_map(const String& p_stacktrace)
    {
        ...
        for (String& st_line : st_lines)
        {
            ...
            int line = (int) match->get_string(group_index + 2).to_int();
            int col = (int) match->get_string(group_index + 3).to_int();

            internal::SourceMap* map = find_source_map(filename);
            if (!map) continue;
            internal::SourcePosition position;
...
#if WINDOWS_ENABLED
            const String restored = internal::PathUtil::combine(internal::PathUtil::dirname(filename), source_root, source).simplify_path().replace("/", "\\");
#else
            const String restored = internal::PathUtil::combine(internal::PathUtil::dirname(filename), source_root, source).simplify_path();
#endif
...
#if DEV_ENABLED
    internal::SourceMap* JavaScriptRuntime::find_source_map(const String& p_filename)
    {
        HashMap<String, internal::SourceMap>::Iterator it = cached_source_maps_.find(p_filename);
